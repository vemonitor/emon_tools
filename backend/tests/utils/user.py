...
from backend.models.db import User, UserCreate, UserUpdate
from tests.utils.user import random_lower_string
from tests.utils.user import random_email
...
def create_random_user(db: Session) -> User:
    """
    Create a random user with a random email and password.
    Args:
    ...
    user = UserController.create_user(session=db, user_create=user_in)
    return user
...
def authentication_token_from_email(
    *, client: TestClient, email: str, db: Session
    ...
    """
    password = random_lower_string()
    user = UserController.get_user_by_email(session=db, email=email)
    if not user:
        user_in_create = UserCreate(email=email, password=password)
        user = UserController.create_user(session=db, user_create=user_in_create)
    else:
