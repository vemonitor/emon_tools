...
def emon_feeds():
    ...
    return AsyncEmonFeeds(BASE_URL, API_KEY)


@pytest.mark.asyncio
async def test_async_list_feeds_empty(emon_feeds):
...
@pytest.mark.asyncio
async def test_async_list_feeds(emon_feeds):
    """Test listing feeds."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
...
@pytest.mark.asyncio
async def test_async_get_feed_fields(emon_feeds):
...
@pytest.mark.asyncio
async def test_async_get_feed_fields_empty(emon_feeds):
    """Test retrieving UUID."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
        mock_request.return_value = {"success": False}
...
@pytest.mark.asyncio
async def test_async_get_feed_fields_invalid(emon_feeds):
    """Test retrieving UUID."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
        mock_request.return_value = {"success": False}
...
@pytest.mark.asyncio
async def test_async_get_feed_meta(emon_feeds):
...
@pytest.mark.asyncio
async def test_async_get_last_value_feed(emon_feeds):
...
@pytest.mark.asyncio
async def test_async_list_inputs(emon_feeds):
    """Test listing inputs."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
...
@pytest.mark.asyncio
async def test_async_list_inputs_process_fields(emon_feeds):
    """Test listing inputs."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
        mock_request.return_value = {
...
@pytest.mark.asyncio
async def test_async_list_inputs_extended_fields(emon_feeds):
    """Test listing inputs."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
        mock_request.return_value = {
...
@pytest.mark.asyncio
async def test_async_get_input_fields(emon_feeds):
    """Test retrieving input fields."""
    with patch.object(
            emon_feeds, "async_request", new=AsyncMock()) as mock_request:
        mock_request.return_value = {
            "success": True, "message": MOCK_INPUT_DETAILS}
        input_details = await emon_feeds.async_get_input_fields("test", "V")
