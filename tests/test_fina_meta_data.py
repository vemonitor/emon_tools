...
This module tests the functionality of the MetaData class,
including validation, serialization,
and computation of derived attributes.
"""
...
class TestMetaData:
    ...
    Tests cover validation of parameters, serialization,
    and derived computations.
    """
    ...
    def test_initialization_invalid_date_order(self):
        """
        Test initialization raises ValueError
        if start_time is not less than end_time.
        """
        with pytest.raises(
                ValueError, match="start_time must be less than end_time"):
            MetaData(
                interval=10, start_time=2000000, npoints=100, end_time=1000000)
    ...
    def test_initialization_invalid_interval(self):
        ...
        """
        with pytest.raises(
                ValueError, match="interval must be a positive integer"):
            MetaData(
                interval=-10,
                start_time=1000000,
                npoints=100,
    ...
    def test_calculate_nb_days(self):
        ...
        """
        start_time = int(
            dt.datetime(2023, 1, 1, tzinfo=dt.timezone.utc).timestamp())
        end_time = int(
            dt.datetime(2023, 1, 3, tzinfo=dt.timezone.utc).timestamp())
        metadata = MetaData(
            interval=10,
    ...
    def test_calculate_nb_days_fractional(self):
        """
        Test calculation of the number of days
        when duration includes fractional days.
        """
        start_time = int(
            dt.datetime(2023, 1, 1, tzinfo=dt.timezone.utc).timestamp())
        end_time = int(
            dt.datetime(2023, 1, 2, 12, tzinfo=dt.timezone.utc).timestamp())
        metadata = MetaData(
            interval=10,
            start_time=start_time,
            npoints=100,
    ...
    def test_string_representation(self):
        ...
        assert str(
            ...
            "{'interval': 10, 'start_time': 1000000, "
            "'npoints': 100, 'end_time': 2000000}")
