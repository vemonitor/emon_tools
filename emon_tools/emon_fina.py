...
class NpFillNan:
    ...
    def fill_nan_values(
        ...
        last_valid_index = len(result_array) - np.argmax(finite_mask[::-1]) - 1  # Index of the last non-NaN value
        ...
        if fill_between:
            ...
            if method == FillNanMethod.FORWARD:
                result_array[first_valid_index:last_valid_index + 1] = NpFillNan.forward_fill_nan(between_slice)
            elif method == FillNanMethod.INTERPOLATE:
                result_array[first_valid_index:last_valid_index + 1] = NpFillNan.interpolate_fill_nan(between_slice)
